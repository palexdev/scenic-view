...
class FullPropertiesDetailPaneInfo extends DetailPaneInfo {
	...
	private void createPropertiesPanel() {
		...
		if (node != null) {
			...
			for (CssMetaData<? extends Styleable, ?> metaData : list) {
				final CssMetaData cssMetaData = (CssMetaData) metaData;
		...
		for (final ObservableValue type : properties.keySet()) {
			orderedProperties.put(properties.get(type), type);
		}
		for (final String type : orderedProperties.keySet()) {
	...
	protected void updateAllDetails() {
		if (orderedProperties != null) {
			for (String s : orderedProperties.keySet()) {
				updateDetail(s, true);
			}
		}
		sendAllDetails();
	...
	@SuppressWarnings({"unchecked"})
	protected void updateDetail(final String propertyName, final boolean all) {
		...
		} else if (value instanceof Background background) {
