...
public class StageControllerImpl implements StageController {
	...
	@Override
	public void setEventDispatcher(final FXConnectorEventDispatcher model2gui) {
	...
	public void update() {
		...
		if (targetScene != null && targetScene.getRoot() == target) {
			...
			if (targetScene.getWindow() instanceof final Stage s) {
			...
			if (!popupWindows.isEmpty()) {
				...
				for (final PopupWindow window : popupWindows) {
	...
	@Override
	public void setSelectedNode(final SVNode svNode) {
		...
		if (add) {
			if (SCUtils.isNormalNode(node)) {
				...
				for (Node child : children) {
					updateListeners(child, add, removeVisibilityListener);
				}
			}
		} else {
			ObservableList<Node> children = ChildrenGetter.getChildren(node);
			...
			for (Node child : children) {
				 ...
				 * reduced by their parent
				 */
				updateListeners(child, add, true);
			}
			children.removeListener(structureInvalidationListener);
...
//        }
	...
	public void updateAnimations() {
		...
		for (final Animation a : animations) {
			svAnimations.add(new SVAnimation(ConnectorUtils.getAnimationUniqueID(a), a));
		}
	...
	private void dispatchEvent(final FXConnectorEvent event) {
		if (dispatcher != null) {
